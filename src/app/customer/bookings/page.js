"use client";
import { useState, Suspense } from "react";
import { useSearchParams, useRouter } from "next/navigation";
import Link from "next/link";
import { useApp } from "@/context/AppContext";
import { ROLES, BOOKING_STATUS, STATUS_TRANSITIONS } from "@/lib/data";
import BookingCard from "@/components/BookingCard";
import Button from "@/components/Button";
import { CheckCircle2, Plus } from "lucide-react";

const TABS = ["All", BOOKING_STATUS.REQUESTED, BOOKING_STATUS.CONFIRMED, BOOKING_STATUS.IN_PROGRESS, BOOKING_STATUS.COMPLETED, BOOKING_STATUS.CANCELLED];

function MyBookingsPage() {
  const { bookings, categories, users, currentUser, updateBookingStatus, dbLoading } = useApp();
  const router = useRouter();
  const searchParams = useSearchParams();
  const justBooked = searchParams.get("booked");
  const [activeTab, setActiveTab] = useState("All");

  if (dbLoading) {
    return (
      <div className="flex items-center justify-center py-24 text-gray-400">
        <div className="w-8 h-8 border-4 border-[#19e65e] border-t-transparent rounded-full animate-spin mr-3" />
        Loading…
      </div>
    );
  }

  if (!currentUser || currentUser.role !== ROLES.CUSTOMER) {
    return (
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        <Button onClick={() => router.push("/auth/login")}>Sign In</Button>
      </div>
    );
  }

  const myBookings = bookings
    .filter((b) => b.customerId === currentUser.id)
    .filter((b) => activeTab === "All" || b.status === activeTab)
    .sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));

  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      {justBooked && (
        <div className="mb-6 bg-green-50 border border-green-200 rounded-2xl px-5 py-4 flex items-center gap-3">
          <CheckCircle2 className="w-5 h-5 text-green-600 shrink-0" />
          <div>
            <p className="font-semibold text-green-800">Booking Requested!</p>
            <p className="text-sm text-green-600">Your booking has been sent to the provider. You&apos;ll be notified when they confirm.</p>
          </div>
        </div>
      )}

      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">My Bookings</h1>
          <p className="text-gray-500 text-sm mt-1">{bookings.filter((b) => b.customerId === currentUser.id).length} total bookings</p>
        </div>
        <Link href="/">
          <Button>
            <Plus className="w-4 h-4" /> New Booking
          </Button>
        </Link>
      </div>

      {/* Tabs */}
      <div className="flex gap-2 overflow-x-auto pb-2 mb-6">
        {TABS.map((t) => {
          const count = t === "All"
            ? bookings.filter((b) => b.customerId === currentUser.id).length
            : bookings.filter((b) => b.customerId === currentUser.id && b.status === t).length;
          return (
            <button
              key={t}
              onClick={() => setActiveTab(t)}
              className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition ${
                activeTab === t
                  ? "bg-[#19e65e] text-white shadow-sm"
                  : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
              }`}
            >
              {t} {count > 0 && <span className="ml-1 text-xs opacity-75">({count})</span>}
            </button>
          );
        })}
      </div>

      {myBookings.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-200">
          <div className="text-4xl mb-3">📋</div>
          <h3 className="font-semibold text-gray-900 mb-1">No bookings yet</h3>
          <p className="text-gray-500 text-sm mb-4">Browse services to make your first booking</p>
          <Link href="/"><Button>Browse Services</Button></Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {myBookings.map((booking) => {
            const cat = categories.find((c) => c.id === booking.categoryId);
            const provider = users.find((u) => u.id === booking.providerId);
            const canCancel = [BOOKING_STATUS.REQUESTED, BOOKING_STATUS.CONFIRMED].includes(booking.status);
            const canReschedule = [BOOKING_STATUS.REQUESTED, BOOKING_STATUS.CONFIRMED].includes(booking.status);
            const canReview = booking.status === BOOKING_STATUS.COMPLETED;
            return (
              <BookingCard
                key={booking.id}
                booking={booking}
                category={cat}
                provider={provider}
                actions={
                  <>
                    <Link href={`/customer/booking/${booking.id}`}>
                      <Button variant="secondary" size="sm">View Details</Button>
                    </Link>
                    {canReschedule && (
                      <Link href={`/customer/booking/${booking.id}/reschedule`}>
                        <Button variant="outline" size="sm">Reschedule</Button>
                      </Link>
                    )}
                    {canCancel && (
                      <Button
                        variant="danger"
                        size="sm"
                        onClick={() => updateBookingStatus(booking.id, BOOKING_STATUS.CANCELLED)}
                      >
                        Cancel
                      </Button>
                    )}
                    {canReview && (
                      <Link href={`/customer/booking/${booking.id}/review`}>
                        <Button variant="success" size="sm">Leave Review</Button>
                      </Link>
                    )}
                  </>
                }
              />
            );
          })}
        </div>
      )}
    </div>
  );
}

export default function MyBookingsPageWrapper() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center py-20 text-gray-500">Loading...</div>}>
      <MyBookingsPage />
    </Suspense>
  );
}
